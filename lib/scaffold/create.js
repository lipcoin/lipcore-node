...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'lipcore-lib': '^' + bitcore.version,
    'lipcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/lipcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the lipcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the lipcore-node directory and configuration
