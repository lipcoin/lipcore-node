...
/**
 * Will return the path and default lipcore-node configuration. It will search for the
 * configuration file in the "~/.lipcoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.lipcoin');
  var defaultConfigFile = path.resolve(defaultPath, './lipcore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/lipcoind')
          }
        }
      }
    };
